...
function setupBot(env) {
  ...
  bot.command('feedback', async (ctx) => {
    ...
    try {
      ...
      feedbackMessage += `Если у тебя есть вопросы, предложения или проблема, пожалуйста, напиши нам!\n\n`;
      feedbackMessage += `Нам важна обратная связь, так мы сможем сделать приложение лучше.`;

      const keyboard = new InlineKeyboard()
        .url('💬 Заполнить форму', 'https://clck.ru/3Rncqs');
  ...
  bot.callbackQuery('show_feedback', async (ctx) => {
    try {
      ...
      const keyboard = new InlineKeyboard()
        .url('💬 Заполнить форму', 'https://clck.ru/3Rncqs').row()
        .text('« Назад', 'back_to_start');
